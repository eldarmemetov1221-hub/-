...
async def main() -> None:
    ...
    ap.add_argument("--threshold", type=float, default=0.3, help="чувствительность смены сцен (0.1–0.6)")
    args = ap.parse_args()
    ...
    print(f"🎬 Видео: {video}\n📝 Текст: {args.script}\n🎙 Голос: {args.voice}")
    print("⏳ Ищу сцены и накладываю озвучку…")

    async def synth(t: str) -> bytes:
        return await synth_edge(t, args.voice, args.rate, args.pitch)

    stats = await videovoice.build_voiced_video(str(video), text, synth, str(out), args.threshold)
    print(f"✅ Готово: {out}")
    print(f"   Сцен найдено: {stats['scenes']}, абзацев озвучено: {stats['paragraphs']}")
